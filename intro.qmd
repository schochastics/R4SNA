...
- `ggraph` and `graphlayouts`: grammar-of-graphics network visualization and additional layout algorithms.
- `tidygraph`: a tidyverse-style interface to `igraph`.
- `netrankr`: a unified framework for centrality indices.
- `signnet`: methods for signed networks.
- `backbone`: backbone extraction from weighted and two-mode networks.
- `RSiena`: stochastic actor-oriented models for longitudinal networks.
- `relevent`: relational event models.
- `egor`: ego-centered network data.
- `networkdata`: a large collection of network datasets used throughout the book.
