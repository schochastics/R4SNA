...
A network is a collection of nodes (or vertices) and edges that connect pairs of nodes. Networks can model relationships and structures in various types of data, such as social networks, transportation networks, and more.

Node (or vertices) are the fundamental unit in a network, representing an entity or object. In network analysis, nodes typically stand for individuals, places, or things depending on the context of the network.

edges indicate connections between two nodes in a network, representing the relationship or interaction between them. Edges can be undirected (bidirectional relationship) or directed (unidirectional relationship).
