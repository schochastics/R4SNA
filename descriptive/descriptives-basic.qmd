...
@fig-flo-overview shows the network. The Medici sit at the structural center,
...
The adjacency matrix of the network can be obtained with the
...
ggraph(g, "stress") +
  geom_edge_link0(
    aes(color = on_path, width = on_path),
    show.legend = FALSE
  ) +
  geom_node_point(shape = 21, size = 8, fill = "#E8813A") +
  geom_node_text(aes(label = name), repel = TRUE) +
...
similar degrees tend to be connected.
...
More informative than the dyad census is the **triad census**, which counts the
occurrence of each of the $16$ possible configurations of edges among three
nodes in a directed network (see @fig-triad-types). Triads are labeled using the
**MAN notation**, often written as `xyzL`, where `x` is the number of
...
#| out-width: "80%"
#| fig-align: center
#| fig-cap: "The 16 possible triads in a directed network, labeled using the MAN (mutual/asymmetric/null) notation."
if (knitr::is_latex_output()) {
  knitr::include_graphics("img/triad-census.pdf")
} else {
  knitr::include_graphics("img/triad-census.png")
}
```
...
follow @faust2007local and perform a singular value decomposition (SVD) on the
normalized triad census matrix.
...
data.frame(
  ...
  labs(
    x = "density",
    y = "First singular vector, multiplied by singular value"
  )
```
