...
The parameter `weights` can be used to artificially "pull" negative ties apart and positive ties together, which can help to visually separate potential groups of positive ties.
...
```{r}
#| label: signed-lap
laplacian_matrix_signed(tribes)
```
...
Real-world networks are rarely perfectly balanced. The question then becomes: *how balanced* is a given network? The `signnet` package offers three methods for measuring the degree of balancedness via `balance_score()`. All return a value between zero (perfectly unbalanced) and one (perfectly balanced).
...
The **frustration** method finds a partition of the network into two groups and counts how many edges violate the balance pattern (negative ties within groups and positive ties between groups). The fewer violations, the more balanced the network [@aref2018measuring]. Since finding the optimal partition is computationally hard (NP-complete), the function uses simulated annealing and returns an upper bound. For exact results, `frustration_exact()` solves the problem via integer programming.

For the `tribes` network, which is not perfectly balanced, the three methods give very different scores:
...
Structural balance theory predicts that a balanced network can be partitioned into groups with positive intra-group ties and negative inter-group ties. Blockmodeling is the computational approach to finding such partitions [@doreian1996partitioning].
...
The function returns a list with two entries: the block membership of each node and the value of $P(C)$, where lower values indicate a better fit. Setting `annealing = TRUE` uses simulated annealing in the optimisation, which generally produces better results at the cost of longer running time.

The block structure can be visualized with `ggblock()`.
...
The traditional blockmodeling assumes a single pattern of positive blocks on the diagonal and negative blocks off the diagonal. But most networks have more complex structures [@doreian2009partitioning]. For instance, two groups might be allies (positive ties between them) while both are enemies of a third group.

The function `signed_blockmodel_general()` allows us to specify arbitrary block structures via the `blockmat` parameter. Each entry in the matrix indicates whether we expect the corresponding block to be positive (1) or negative (-1). In the following we construct a synthetic network with three groups, with a structure which cannot be captured by the traditional blockmodel but can be specified in the general blockmodel.
