...
Prediction power heatmaps for verttex variables; darker cells indicate stronger predictive power (lower $EH(Z \mid X,Y)$).
...
Prediction power heatmaps for dyad variables; darker cells indicate stronger predictive power (lower $EH(Z \mid X,Y)$). 
:::
...
(p7 | plot_spacer())
```
Prediction power heatmaps for triad variables; darker cells indicate stronger predictive power (lower $EH(Z \mid X,Y)$).
::: 
...
```
Association graph for dyad variables (threshold $J > 0.1$). The variables `friend`, `cowork`, and `advice` (highlighted in red) form a tightly connected group, suggesting a strong dependence structure and motivating the conditional independence model $\text{friend} \perp \text{cowork} \mid \text{advice}$.
::: 
