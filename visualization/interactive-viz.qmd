...
As in the previous chapters, we use the Game of Thrones character interaction network from season 1.
...
#| fig-cap: "GoT network rendered with `visNetwork`."

## visNetwork expects columns: label, group, value
V(gotS1)$label <- V(gotS1)$name
V(gotS1)$group <- V(gotS1)$clu
V(gotS1)$value <- V(gotS1)$size

vis_data <- toVisNetworkData(gotS1)
...
```{r}
#| label: fig-networkd3
#| fig-cap: "GoT network rendered with `networkD3`."
d3_data <- igraph_to_networkD3(gotS1,
  group = as.numeric(V(gotS1)$clu)
)
...
```{r}
#| label: fig-threejs
#| fig-cap: "GoT network in 3D with `threejs`."
graphjs(gotS1,
  vertex.size = V(gotS1)$size * 0.2,
  vertex.color = got_palette[as.numeric(V(gotS1)$clu)]
)
```
...
```{r}
#| label: fig-g6r-first
#| fig-cap: "GoT network with `g6R` using default settings."
g6_igraph(gotS1)
```
...
#| fig-cap: "GoT network with D3 force-directed layout."
g6_igraph(gotS1) |>
...
#| fig-cap: "GoT network with Fruchterman-Reingold layout."
g6_igraph(gotS1) |>
...
#| fig-cap: "GoT network with circular layout."
g6_igraph(gotS1) |>
...
#| fig-cap: "GoT network with radial layout centered on Eddard."
g6_igraph(gotS1) |>
  g6_layout(radial_layout(focusNode = "Eddard")) |>
  g6_behaviors(drag_canvas(), zoom_canvas())
```
...
#| fig-cap: "GoT network with concentric layout sorted by degree."

## g6R needs a 'degree' attribute on the nodes
V(gotS1)$degree <- degree(gotS1)
...
#| fig-cap: "GoT network styled via igraph vertex and edge attributes."

## Map community membership to fill color
V(gotS1)$fill <- got_palette[as.numeric(V(gotS1)$clu)]
V(gotS1)$stroke <- "#555555"
V(gotS1)$lineWidth <- 1.5

## Style edges
E(gotS1)$stroke <- "#cccccc"
...
#| fig-cap: "GoT network styled with `g6_options()`."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_options(
    node = node_options(
...
```{r}
#| label: fig-g6r-nav
#| fig-cap: "GoT network with basic navigation behaviors."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(drag_canvas(), zoom_canvas())
```
...
#| fig-cap: "GoT network with drag, hover, and click-select behaviors."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(
    drag_canvas(),
...
```{r}
#| label: fig-g6r-lasso
#| fig-cap: "GoT network with lasso and brush selection."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(
    drag_canvas(),
...
#| fig-cap: "GoT network with a minimap for navigation."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(drag_canvas(), zoom_canvas(), drag_element_force()) |>
  g6_plugins(minimap())
...
#| fig-cap: "GoT network with hover tooltips."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
...
```{r}
#| label: fig-g6r-hull
#| fig-cap: "GoT network with hull grouping by community."

## Split nodes by community membership
clu_members <- split(V(gotS1)$name, V(gotS1)$clu)
...
g6_igraph(gotS1) |>
  ...
  g6_plugins(
    hull(
      ...
      fill = got_palette[1], stroke = got_palette[1]
    ),
    hull(
      key = "hull-2", members = clu_members[["2"]],
      fill = got_palette[2], stroke = got_palette[2]
    ),
    hull(
      key = "hull-3", members = clu_members[["3"]],
      fill = got_palette[3], stroke = got_palette[3]
    )
  )
```
...
#| fig-cap: "GoT network with bubble-set grouping."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(drag_canvas(), zoom_canvas(), drag_element_force()) |>
  g6_plugins(
...
#| fig-cap: "GoT network with fisheye distortion."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(drag_canvas(), zoom_canvas()) |>
  g6_plugins(fish_eye())
...
#| fig-cap: "GoT network with legend showing community membership."
g6_igraph(gotS1) |>
  g6_layout(d3_force_layout()) |>
  g6_behaviors(drag_canvas(), zoom_canvas(), drag_element_force()) |>
...
```{r}
#| label: fig-g6r-final
#| fig-cap: "Complete interactive GoT character network with `g6R`."
