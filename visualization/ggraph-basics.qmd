...
We use the network of characters in the first season of Game of Thrones (GoT) as a running example. The dataset is included in the `networkdata` package.
...
Most edge geoms come in three flavors. The standard `geom_edge_link()` draws 100 dots on each edge compared to only two dots (the endpoints) in `geom_edge_link0()`. This is done to allow gradients along the edge (@fig-got-plot-grad). 
...
The default arrowhead type is "open", yet "closed" usually has a nicer appearance (@fig-arrow).
...
You do not necessarily need to set them, since `ggraph` can take care of it automatically (@fig-no-scales).
```{r}
#| label: fig-no-scales
...
as a named vector. The difference between unnamed and named palettes is shown in @fig-got-plot-pal2.
```{r}
#| label: named_palette
got_palette2 <- c(
...
The function offers all palettes available at [colorbrewer2.org](http://colorbrewer2.org/), as shown in @fig-got-plot-brewer. 
...
We start with a simple plot without any styling (@fig-polblogs1).
```{r}
#| label: fig-polblogs1
...
There is obviously a lot missing. First, we delete all isolates and plot again (@fig-polblogs2).

```{r}
#| label: fig-polblogs2
#| fig-cap: "Political blogs network after removing isolates."
#| fig-width: 10
#| fig-height: 8
...
The original does feature a small disconnected component, but we remove this here (@fig-polblogs2a).

```{r}
#| label: fig-polblogs2a
#| fig-cap: "Political blogs network after removing the small disconnected component."
#| fig-width: 10
#| fig-height: 8
...
Let us now start with some styling of the nodes (@fig-polblogs3).
...
we map the degree to node size (@fig-polblogs4).

```{r}
#| label: fig-polblogs4
#| fig-cap: "Political blogs network with corrected colours and degree-based node size."
#| fig-width: 10
#| fig-height: 8
ggraph(lay) +
    geom_edge_link0(
...
The node sizes are also not that satisfactory, so we fix the range with `scale_size()` (@fig-polblogs5).

```{r}
#| label: fig-polblogs5
#| fig-cap: "Political blogs network with adjusted node size range."
#| fig-width: 10
#| fig-height: 8
ggraph(lay) +
    geom_edge_link0(
...
This new variable is mapped to the edge color (@fig-polblogs6).
...
#| fig-width: 10
#| fig-height: 8
el <- as_edgelist(polblogs, names = FALSE)
...
Similar to the node colors, we add a `scale_edge_colour_manual()` to adjust the edge colors (@fig-polblogs7).

```{r}
#| label: fig-polblogs7
#| fig-cap: "Political blogs network with manually set edge colours."
#| fig-width: 10
#| fig-height: 8
ggraph(lay) +
    geom_edge_link0(
...
In that way, the mixed edges are getting plotted below (@fig-polblogs8).
...
Now let us just add the `theme_graph()` to finalize the plot (@fig-polblogs9).
```{r}
#| label: fig-polblogs9
