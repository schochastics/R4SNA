...
### Model Estimation and Simulation {#sec-ergmest}
...
#### Model Instability and Degeneracy {.unnumbered}
...
::: {.callout-tip}
# Running MCMC Diagnostics for ERGM
...
This goodness-of-fit check produces plots for several structural features, including both modeled and un-modeled terms:

- **Model statistics**: the sufficient statistics explicitly specified in the model formula (e.g., `edges`, `nodematch`, `gwesp`).
- **Degree distribution**: the number of ties per node,
- **Geodesic distances**: shortest paths between nodes,
- **Edgewise shared partners (ESP)**: the number of common neighbors shared by directly connected pairs of nodes.


ESP should not be interpreted as a fit criterion in this case, since it is directly modeled via the `gwesp` term. Including it in GOF checks may simply reflect that the model was designed to reproduce it.

The GOF plots (see @fig-gof) compare statistics from the observed network (black lines or blue diamonds) to distributions of the same statistics across networks simulated from the model:

- **Model statistics**: This plot shows whether the model reproduces the statistics it was explicitly fit to match. Each boxplot represents the distribution of simulated values for a modeled term (e.g., `edges`, `nodematch.practice`, `gwesp`), and the blue diamond marks the observed value. Good fit is indicated when the diamond lies near the center of the box. This panel essentially checks if maximum likelihood estimation succeeded.

- **Degree distribution**: The model accurately captures the variability in the number of ties per node if the observed degree frequencies fall within the simulated distribution at most points.

- **Geodesic distances**: If the model reflects the observed network's overall connectivity, the distribution of shortest path lengths should align closely with those from the simulations.

- **Edgewise shared partners**: This panel can typically indicate clustering tendencies, but in this case is not meaningful, since `gwesp` was already included to model triadic closure.

::: {.callout-note}
# Note: Interpreting Black Lines and Blue Diamonds in GOF Plots
...
We can also look at the printed GOF summary:
