...
To illustrate how we can test for homophily using a non-paramtric approach, we turn to another data set with available node attributes.

### Test 1: Friendship based on gender {.unnumbered}
First, we examine gender-based homophily in a friendship network among corporate lawyers from the Lazega dataset [@lazega2001collegial]. We load the data as a graph object from  the package **networkdata**:

```{r}
law_friends_g <-  networkdata::law_friends
```

We are now testing hte following
...
### Test 2: Cowork based on law practice {.unnumbered}
