...
n observed network features, thereby enabling controlled comparisons.
...
# Create an observed network
set.seed(123)
obs_graph <- sample_smallworld(1, 50, nei = 3, p = 0.15)

# Compute centrality measure
obs_measure <- centralization.degree(obs_graph)$centralization

# Simulate null distribution
n_sim <- 500
null_vals <- replicate(n_sim, {
  g_sim <- sample_gnm(vcount(obs_graph), ecount(obs_graph))
  centralization.degree(g_sim)$centralization
})

# Plot 1: observed network
p1 <- ggraph(obs_graph, layout = "fr") +
  geom_edge_link(color = "grey") +
  geom_node_point(size = 2) +
  theme_void() +
  ggtitle("observed network") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

# Plot 2: histogram of null distribution
df <- data.frame(centralization = null_vals)
p2 <- ggplot(df, aes(x = centralization)) +
  geom_histogram(binwidth = 0.02, fill = "skyblue", color = "black") +
  geom_vline(xintercept = obs_measure, linetype = "dashed", color = "tan2", size = 1) +
  labs(title = "distribution of measure under null", x = NULL, y = NULL) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
# Add annotation (curved arrow + label)
grid.newpage()
grid.draw(ggplotGrob(combined))
grid.curve(x1 = 0.42, y1 = 0.65, x2 = 0.57, y2 = 0.65,
           curvature = -0.3, arrow = arrow(length = unit(0.03, "npc")),
           gp = gpar(col = "tan2", lwd = 2))
grid.text("measure of interest", x = 0.5, y = 0.7, gp = gpar(col = "tan2", fontface = "italic"))
```
