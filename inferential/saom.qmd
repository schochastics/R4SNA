...
These effects capture the idea that individuals adjust their behavior based on their peers:

- Average Similarity
  This effect promotes alignment of an actor’s behavior with that of their network alters:
  ...
  $$
  s_i^{\text{avsim}}(x, z, v) = \frac{1}{x_{i+}} \sum_{j=1}^n x_{ij} (\text{sim}_z(ij) - \overline{\text{sim}_z})
  $$

  Here, $R_z$ is the range of the behavior variable, and $x_{i+}$ is the number of ties ego sends.

- Total Similarity 
  Similar to the average similarity effect but weighted by number of alters:

  $$
  s_i^{\text{totsim}}(x, z, v) = \sum_{j=1}^n x_{ij} (\text{sim}_z(ij) - \overline{\text{sim}_z})
  $$

  This emphasizes the overall strength of behavioral alignment with peers.

For both the average  and total effects, the estimated parameter reflects the strength and direction of social influence, that is, how actors adjust their behavior in relation to the behaviors of their connected peers.

- A positive parameter estimate means that actors are more likely to adjust their behavior to become more similar to those they are connected to. In other words, the model captures conformity or peer influence. The higher the similarity between ego and alters, the higher the objective function, making such changes more likely.

- A negative parameter estimate would indicate that actors tend to diverge from their peers, becoming less similar over time. This could suggest differentiation or opposition in behavior, although negative influence parameters are rare and often theoretically unexpected.
...
- Outdegree effect 
  ...
  $$
  s_i^{\text{out}}(x, z, v) = z_i \sum_{j=1}^n x_{ij}
  $$
...
- Indegree effect 
  ...
  $$
  s_i^{\text{ind}}(x, z, v) = z_i \sum_{j=1}^n x_{ji}
  $$
