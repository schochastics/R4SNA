...
@tbl-ergmsaom shows the core differences between ERGMs and SAOMs. While ERGMs model the
global structure of a network at a single point in time, SAOMs treat network
evolution as a dynamic process driven by actors making sequential decisions. In other words, SAOMs provide a complementary dynamic framework for modeling the mechanisms of network change rather than only the final observed structure.
...
subset of 50 pupils, we model friendship dynamics across all three observed waves, using all 129 pupils and including relevant actor-level covariates such as gender and smoking behavior.
...
We load the data from the `networkdata` package and extract an `igraph` object for
each time period.
...
```
These `igraph` objects are the network snapshots used in the SAOM examples below. We
...
particular time point, that is $y_{ij}(t) = 1$ if actor $i$ has a tie to actor
$j$ at time $t$, and $y_{ij}(t) = 0$ otherwise.
...
observe a triadic closure (i.e., if $i$ is tied to $j$ and $j$ to $\ell$, then
$i$ may become tied to $\ell$), we cannot know whether it reflects an
intentional closure or merely a residual pattern without knowing the order in
which ties appeared.
...
$y_{ij}(t) \in \{0,1\}$ indicating whether a tie from actor $i$ to actor $j$ is
present at time $t$.
...
::: {.callout-note}
A continuous-time Markov chain can be understood as a process that waits in a
state for a random holding time and then jumps to a new state. The cat example
in @sec-apx-cat illustrates this intuition.
:::

### Network States and the Markov Property
...
In the sections that follow, we explore how each of these components is
specified in SAOMs, including how actors are selected to make changes and how
tie updates are determined.
...
$\{Y(t), t \geq 0\}$ changes over time through a sequence of small, actor-driven
modifications, where individual ties $y_{ij}(t)$ may be created or dissolved.
...
- **Latent process**: the underlying continuous-time evolution, consisting of a
  sequence of micro-steps in which actors stochastically update individual ties
  $y_{ij}(t)$ based on the current network state $Y(t)$.
...
$$
where $Y(t)$ denotes the network state (i.e., adjacency matrix) at time $t$, and
$y, y' \in \mathcal{Y}$ are particular network configurations at times $t_i$ and
$t_j$, respectively.
...
2. **One tie at a time**: The selected actor $i$ considers one outgoing tie
   ...
   ($y_{ij}(t) = 1$), dissolve it ($y_{ij}(t) = 0$), or leave it unchanged. Only
   one tie variable can change per step, which keeps the space of possible
...
$\{Y^{(a)}\}_{a=1}^{A}$, each differing from the current network $Y(t)$ by at most one outgoing tie $y_{ij}(t)$. The actor may form a new tie, dissolve an existing tie, or choose to make no change.
...
We now make explicit how the deterministic component $F_{ia}$ is constructed. It
is given by an **objective function**:
...
The objective function for actor $i$ evaluating a candidate network $Y^{(a)}$ is
given by:
...
where $\theta_k$ is the parameter for effect $k$, and $s_{ik}(Y^{(a)})$ is the
value of effect $k$ evaluated for actor $i$ in the candidate network $Y^{(a)}$.
Each effect enters the objective function with an associated parameter to be
estimated from data. The weighted sum of these effects (together with a
stochastic component) determines the actor's utility for each potential tie
change, guiding the probabilistic decision-making process at the heart of the
SAOM framework.
...
triadic closure. The transitive effect reflects a tendency toward forming closed
triads. If actor $i$ sends a tie to $j$, and $j$ sends a tie to $h$, then actor $i$ is more likely to send a tie to $h$.
...
tendency of actors to form circular structures in the network. Specifically, it
counts the number of directed cycles of length three involving actor $i$:
...
We denote covariate information by $v$. Depending on the context, $v$ may
include:

- **Actor-level covariates**: $v_i$ (e.g., age, gender, smoking status),
- **Dyadic covariates**: $v_{ij}$ (e.g., geographic distance or similarity
  between actors $i$ and $j$).
...
attribute-driven mechanisms. While only a few examples are presented here, SAOMs
provide a wide range of endogenous and exogenous effects that can be combined to
test theoretically meaningful processes of network evolution.
...
parameter vector $\theta = (\theta_1, \dots, \theta_K)$ is assumed to be
...
## Extending SAOMs: The Co-Evolution of Networks and Behavior
...
framework captures how actors form, maintain, or dissolve ties over time. It can
include covariates (such as gender or smoking status) to explain **social
selection**, meaning that actor attributes influence the likelihood of tie
formation or dissolution. In this case, actor attributes are treated as covariates rather than outcomes; they may be fixed or changing over time, but their evolution is not modeled.
...
behaviors change over time. This makes it possible to study **social
influence**, where individuals may adopt behaviors similar to those of their
network peers. The model thus captures both the selection of network ties based
on covariates and the influence of peers on individual behavior.
...
We first convert the 3D network array `net_array` into a `sienaDependent` object
using the `sienaDependent()` function. This step tells `RSiena` that the array
...
We begin by calling `getEffects()` on our `data_mod1` object to create a default
effects object. This object holds all possible effects for the given data
structure and is used as a base for customization. You can have a look at these
effects directly by calling it.
...
A nicer summary of the model is obtained by running the following, which renders
the table of results inline (including significance stars for easy
interpretation).
...
if (knitr::is_latex_output()) {
  cat(":::{=latex}", readLines("saom_1.tex"), ":::", sep = "\n")
} else {
  lines <- readLines("saom_1.html")
  lines <- sub("^[[:space:]]+", "", lines)
  lines <- gsub("&#134;?", "&dagger;", lines)
  cat(":::{=html}", lines, ":::", sep = "\n")
}
```

We now interpret the results from Model 1, beginning with an assessment of
...
- **Rate 1**: Estimate = 8.580 (SE = 0.698)\
  On average, each actor had about 9 opportunities to change a tie between waves
  1 and 2.

- **Rate 2**: Estimate = 7.238 (SE = 0.605)\
  On average, each actor had about 7 opportunities to change a tie between waves
  2 and 3.
...
```{r}
#| message: false
# Extract sex and recode to numeric: 1 = Female, 0 = Male
...
#| results: asis
if (knitr::is_latex_output()) {
  cat(":::{=latex}", readLines("saom_2.tex"), ":::", sep = "\n")
} else {
  lines <- readLines("saom_2.html")
  lines <- sub("^[[:space:]]+", "", lines)
  lines <- gsub("&#134;?", "&dagger;", lines)
  cat(":::{=html}", lines, ":::", sep = "\n")
}
...
which is coded as `0` for boys and `1` for girls and is centered around its mean
value.
...
behavior are treated as jointly dependent processes. The network process models
changes in friendship ties, while the behavioral process models changes in
actors' smoking behavior over time.

In this specification, smoking enters the model in two ways. First, smoking is
used in the network objective function to test **selection** effects: whether
actors form or maintain ties based on smoking behavior. Second, smoking is
modeled as a behavioral dependent variable to test **influence** effects:
whether actors adjust their smoking behavior in response to their friends.

> *Key point: `smoking_beh` is no longer only a covariate. It is a behavioral
> dependent variable, while also being used to explain network selection.*
...
#| eval: !expr knitr::is_latex_output()
#| echo: !expr knitr::is_latex_output()
siena_table(saom_coev, type = "tex", sig = TRUE)
```
...
#| results: asis
if (knitr::is_latex_output()) {
  cat(":::{=latex}", readLines("saom_coev.tex"), ":::", sep = "\n")
} else {
  lines <- readLines("saom_coev.html")
  lines <- sub("^[[:space:]]+", "", lines)
  lines <- gsub("&#134;?", "&dagger;", lines)
  cat(":::{=html}", lines, ":::", sep = "\n")
}
...
Before interpreting the estimates, we first check convergence. In this model, the overall maximum convergence ratio exceeds the recommended threshold, indicating that the model has not fully converged. Therefore, the estimates below should be treated as illustrative rather than definitive.

With that caution in mind, the co-evolution model suggests the following patterns:
...
- **Behavioral dynamics (smoking)**
  - Negative linear and positive quadratic shape effects describe the baseline tendency in smoking behavior, but this pattern should be interpreted cautiously because the model has not fully converged.
...
To improve convergence, one can adjust the estimation settings (e.g., increase
the number of simulation iterations via `n3`, modify phase lengths, or tune
other algorithm parameters). Details on these options can be found in the RSiena
Manual [@ripley2011manual], particularly in the sections on:
...
Formally, the goodness-of-fit assessment evaluates the null hypothesis that
the fitted SAOM is capable of generating data with structural or behavioral
properties similar to those observed. The plots compare observed statistics
with the distributions of the same statistics across simulations from the
fitted model. `sienaGOF()` also reports an overall Mahalanobis-distance-based
$p$-value. A relatively large $p$-value (commonly greater than 0.05) indicates
no evidence of substantial discrepancy between the observed and simulated
statistics, whereas a small $p$-value suggests that the model does not
adequately reproduce the assessed feature.
...
```{r}
#| label: fig-gof-indegree
#| fig-cap: "Goodness of fit for the indegree distribution in Model 3 (network and behavioral co-evolution)."
#| message: false
#| warning: false
...
@fig-gof-indegree compares the observed indegree distribution with the
distribution obtained from the simulated networks. The overall goodness-of-fit
test gives $p = 0.428$. Because this value is well above 0.05, there is no
evidence of substantial lack of fit with respect to the indegree distribution.
Thus, the model appears to reproduce this aspect of the friendship network
reasonably well.
...
```{r}
#| label: fig-gof-smoking
#| fig-cap: "Goodness of fit for the smoking behavior distribution in Model 3 (network and behavioral co-evolution)."
#| message: false
#| warning: false
...
@fig-gof-smoking compares the observed distribution of smoking behavior with
the distributions obtained from the simulations. One statistic is not plotted
because its variance is zero, meaning that this statistic does not vary across
the simulations and therefore cannot be meaningfully displayed in the
goodness-of-fit plot. This does not by itself indicate poor model fit.

For the statistics that can be assessed, the overall goodness-of-fit test gives
$p = 0.439$. Because this is above 0.05, there is no evidence of substantial
lack of fit for the smoking behavior distribution based on this test. The
simulated behavioral distributions therefore appear broadly consistent with the
...
Because Model 3 has not fully converged, however, these goodness-of-fit results
should be interpreted cautiously and regarded as provisional until satisfactory
