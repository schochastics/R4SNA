...
This section focuses on a few commonly used endogenous effects; those based
...
$$
s_i^{\text{rec}}(x') = \sum_j x'_{ij} x'_{ji}
$$
...
$$
s_i^{\text{trans}}(x') = \sum_{j,h} x'_{ij} x'_{ih} x'_{hj}
$$
...
$$
s_i^{\text{cyc}}(x') = \sum_{j,h} x'_{ij} x'_{jh} x'_{hi}
$$
...
In Stochastic Actor-Oriented Models (SAOMs), each parameter $\theta_k$ quantifies
...
- If $\theta_k < 0$, configurations with lower values of $s_{ik}(x')$ are more
...
siena_table(saom_1, type = "tex", sig = TRUE)
```
...
siena_table(saom_2, type = "tex", sig = TRUE)
```
...
$$
where $R_v$ refers to the range of the covariate $v$. Substituting the estimated
...
### Network and Behavioral Objective Functions
