...
Assume we have $1,\dots,n$ actors. A potential
relational event at time $t$ is an ordered pair $(i,j)$ with $i \neq j$, where
...
$$
where $Y_{ij}(t) \in \{0,1\}$ is an indicator variable such that
$Y_{ij}(t) = 1$ if the event $i \rightarrow j$ occurs at time $t$, and
$Y_{ij}(t) = 0$ otherwise.
...
{\sum_{p \neq q} N_{pq}(t-1)}
$$
where $N_{ij}(t) = \sum_{\tau \le t} Y_{ij}(\tau)$ is the cumulative count of
events from actor $i$ to actor $j$ up to time $t$. Thus, $N_{ij}(t-1)$ represents the history of interactions from $i$ to $j$
prior to the current event at time $t$.
