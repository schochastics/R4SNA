...
Below is a hypothetical transition matrix $P$ for the cat's behavioral states (row-normalized).
Each row corresponds to the current state, and each column to the next state:

  |           | Sleep | Eat  | Play | Chaos |
  | --------- | :---: | :--: | :--: | :---: |
  | **Sleep** | 0.00  | 0.33 | 0.40 | 0.27  |
  | **Eat**   | 0.41  | 0.00 | 0.47 | 0.12  |
  | **Play**  | 0.04  | 0.62 | 0.00 | 0.34  |
  | **Chaos** | 0.46  | 0.23 | 0.31 | 0.00  |
...
# Corrected row-stochastic transition matrix P (rows sum to 1)
P <- matrix(
  c(0.00, 0.33, 0.40, 0.27,
    0.41, 0.00, 0.47, 0.12,
    0.04, 0.62, 0.00, 0.34,
    0.46, 0.23, 0.31, 0.00),
  nrow = 4, byrow = TRUE,
...
# Build edge list from the matrix (drop zero-probability self-loops)
transitions <- expand.grid(from = state_names, to = state_names, stringsAsFactors = FALSE)
transitions$prob <- apply(transitions, 1, function(row) P[row["from"], row["to"]])
transitions <- transitions[transitions$prob > 0, ]
...
ggraph(g, layout = "circle") +
  geom_edge_arc(aes(label = prob),
                ...
                show.legend = FALSE) +
  geom_node_circle(aes(r = 0.15), fill = "lightgrey", color = "black") +
